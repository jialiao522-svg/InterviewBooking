...
export class GoogleAuthRequiredError extends Error {
  constructor(public readonly tokenPath: string) {
    super(
      `No Google OAuth token found at ${tokenPath}. Run the authorization consent flow first.`,
    );
...
function loadToken(tokenPath: string): StoredGoogleToken {
 ...
 *
 * Requests both the Gmail send scope and the Calendar events scope in one
 * consent screen, since a single stored token is shared by sendEmail and
 * createCalendarEvent — Calendar attendee invites must be sent as this real
 * Google account, not the Calendar service account, which Google forbids
 * from inviting attendees without Workspace Domain-Wide Delegation.
 */
export async function runOAuthConsentFlow(
  ...
  const authUrl = oauth2Client.generateAuthUrl({
    access_type: "offline",
    scope: [GMAIL_SEND_SCOPE, CALENDAR_EVENTS_SCOPE],
    prompt: "consent",
  });
...
export async function ensureGoogleAuthorized(
  config: GmailOAuthConfig = getGmailOAuthConfig(),
): Promise<void> {
 ...
 */
function resolveToken(config: GmailOAuthConfig): StoredGoogleToken {
  if (hasStoredToken(config)) {
  ...
  throw new GoogleAuthRequiredError(config.tokenPath);
}
